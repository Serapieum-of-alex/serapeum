...
def test_create_datasource(
        manual_pdf: List[str], num_manual_pages: int
):
    datasource = DataSource(
        dtype="pdf", file_paths=manual_pdf
    )
...
def test_create_splitter(manual_pdf: List[str]):
    ...
    datasource.create_splitter(model_id="sentence-transformers/all-MiniLM-L12-v2", chunk_size=256, overlap=None)
