...
from serapeum.embeddings.openai import OpenAIEmbedding  # pants: no-infer-dep
...
def test_resolve_embed_model(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(
        "serapeum.embeddings.huggingface.HuggingFaceEmbedding.__init__",
        mock_hf_embeddings,
    )
    monkeypatch.setattr(
        "serapeum.embeddings.openai.OpenAIEmbedding.__init__",
