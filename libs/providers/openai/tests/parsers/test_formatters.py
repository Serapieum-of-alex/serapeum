...
class TestHelperFunctions:
    ...
    def test_should_null_content_assistant_without_any_tool_info(self) -> None:
        ...
        message = Message(
            role=MessageRole.ASSISTANT, chunks=[TextChunk(content="hello")]
        )
        result = _should_null_content(message, has_tool_calls=False)
        assert result is False, "Expected False when assistant has no tool-related data"
...
class TestChatFormat:
    ...
    def test_image_with_url(self) -> None:
        ...
        assert (
            result["image_url"]["url"] == "https://example.com/img.png"
        ), "Expected URL to be passed through"
        assert (
            "detail" not in result["image_url"]
    ...
    def test_image_with_url_and_detail(self) -> None:
        ...
        result = ChatFormat.image(block)
        assert result["image_url"]["detail"] == "low", "Expected detail to be included"
    ...
    def test_image_inline(self) -> None:
        ...
        result = ChatFormat.image(block)
        assert (
            result["image_url"]["url"] == "data:image/png;base64,abc123"
    ...
    def test_audio(self) -> None:
        ...
        assert (
            result["input_audio"]["data"] == "YXVkaW9kYXRh"
        ), "Expected base64 audio data"
        assert (
            result["input_audio"]["format"] == "mp3"
...
class TestResponsesFormat:
    ...
    def test_image_inline(self) -> None:
        ...
        result = ResponsesFormat.image(block)
        assert (
            result["image_url"] == "data:image/png;base64,abc123"
...
class TestChatMessageConverterBuild:
    ...
    def test_reference_audio_id_ignored_for_non_assistant(self) -> None:
        ...
        result = ChatMessageConverter(message).build()
        assert "audio" not in result, "Expected no audio key for non-assistant message"
    ...
    def test_system_message_string_content(self) -> None:
        ...
        message = Message(
            role=MessageRole.SYSTEM, chunks=[TextChunk(content="you are helpful")]
        )
        result = ChatMessageConverter(message).build()
        assert result == {
            "role": "system",
    ...
    def test_audio_block(self) -> None:
        ...
        assert (
            audio_items[0]["input_audio"]["format"] == "mp3"
    ...
    def test_image_inline_data(self) -> None:
        ...
        assert (
            img_items[0]["image_url"]["url"] == "data:image/png;base64,abc123"
        ), "Expected data URI for inline image"
    ...
    def test_assistant_with_text_and_tool_calls_keeps_content(self) -> None:
        ...
        result = ChatMessageConverter(message).build()
        assert (
            result["content"] == "Here are results"
    ...
    def test_system_role_rewritten_to_developer_for_o3_mini(self) -> None:
        ...
        message = Message(
            role=MessageRole.SYSTEM, chunks=[TextChunk(content="be helpful")]
        )
        result = ChatMessageConverter(message, model="o3-mini").build()
        assert (
    ...
    def test_system_role_not_rewritten_for_o1_mini(self) -> None:
        ...
        assert result["role"] == "system", "Expected system role to remain for o1-mini"
    ...
    def test_system_role_not_rewritten_for_gpt4o(self) -> None:
        ...
        assert result["role"] == "system", "Expected system role to remain for gpt-4o"
    ...
    def test_drop_none_false_keeps_none_values(self) -> None:
        ...
        assert result["content"] is None, "Expected content to be None (not stripped)"
    ...
    def test_legacy_tool_calls_not_merged_when_tool_call_blocks_exist(self) -> None:
        ...
        message = Message(
            ...
            additional_kwargs={
                "tool_calls": [
                    {"id": "tc_legacy", "type": "function", "function": {"name": "f"}}
                ],
            },
        )
        result = ChatMessageConverter(message).build()
        ...
        assert (
            result["tool_calls"][0]["id"] == "call_chunk"
    ...
    def test_unsupported_block_type_raises_value_error(self) -> None:
        ...
        message = Message(
            role=MessageRole.ASSISTANT, chunks=[TextChunk(content="placeholder")]
        )
    ...
    def test_thinking_block_skipped_in_chat(self) -> None:
        ...
        message = Message(
            role=MessageRole.ASSISTANT,
            chunks=[
                ThinkingBlock(content="reasoning", additional_information={"id": "t1"}),
...
class TestToOpenaiResponsesMessageDict:
    ...
    def test_assistant_text_returns_dict(self) -> None:
        ...
        message = Message(
            role=MessageRole.ASSISTANT, chunks=[TextChunk(content="response")]
        )
        result = ResponsesMessageConverter(message).build()
    ...
    def test_thinking_block_with_valid_content(self) -> None:
        ...
        assert result[0]["type"] == "reasoning", "Expected first item to be reasoning"
        assert (
    ...
    def test_thinking_block_without_content_is_skipped(self) -> None:
        ...
        assert isinstance(result, dict), "Expected dict when thinking block is skipped"
        ...
        assert (
            text_items[0]["text"] == "answer"
        ), "Expected answer text in output_text item"
    ...
    def test_tool_call_blocks_produce_function_call_list(self) -> None:
        ...
        assert result[0]["type"] == "function_call", "Expected function_call type"
    ...
    def test_tool_call_with_reasoning(self) -> None:
        ...
        assert result[1]["type"] == "function_call", "Expected function_call second"
    ...
    def test_tool_message_with_alternative_call_id_key(self) -> None:
        ...
        result = ResponsesMessageConverter(message).build()
        assert result["call_id"] == "call_2", "Expected call_id from alternative key"
    ...
    def test_system_message_string_content(self) -> None:
        ...
        assert result["content"] == "be helpful", "Expected string content for system"
    ...
    def test_audio_block_raises_in_responses_api(self) -> None:
        ...
        with pytest.raises(
            ValueError, match="Audio blocks are not supported in the Responses API"
        ):
            ResponsesMessageConverter(message).build()
...
class TestToOpenaiMessageDicts:
    ...
    def test_responses_path_delegates_to_responses_dict(self) -> None:
        ...
        developer_msgs = [
            m for m in result if isinstance(m, dict) and m.get("role") == "developer"
        ]
        assert (
            len(developer_msgs) == 1
    ...
    def test_responses_path_string_results_wrapped(self) -> None:
        ...
        messages = [
            m for m in result if isinstance(m, dict) and m.get("role") == "user"
        ]
    ...
    def test_responses_path_multiple_messages_mix(self) -> None:
        ...
        messages = [
            Message(
                role=MessageRole.SYSTEM, chunks=[TextChunk(content="system prompt")]
            ),
...
class TestChatMessageConverterMethods:
    ...
    def test_init_stores_message_and_defaults(self) -> None:
        ...
        assert (
            conv._drop_none is False
        ), f"Expected drop_none=False, got {conv._drop_none}"
        assert conv._content == [], f"Expected empty content, got {conv._content}"
        assert (
            conv._content_txt == ""
        ), f"Expected empty content_txt, got {conv._content_txt!r}"
        assert (
            conv._tool_call_dicts == []
        ), f"Expected empty tool_call_dicts, got {conv._tool_call_dicts}"
    ...
    def test_init_with_all_params(self) -> None:
        ...
        assert (
            conv._drop_none is True
        ), f"Expected drop_none=True, got {conv._drop_none}"
    ...
    def test_try_audio_reference_assistant_with_audio_id(self) -> None:
        ...
        assert result["audio"] == {
            "id": "audio_123"
        }, f"Unexpected audio: {result['audio']}"
    ...
    def test_process_blocks_text_chunk(self) -> None:
        ...
        assert (
            len(conv._content) == 1
        ), f"Expected 1 content item, got {len(conv._content)}"
        assert conv._content[0] == {
            "type": "text",
            "text": "hello",
        }, f"Unexpected content dict: {conv._content[0]}"
        assert (
            conv._content_txt == "hello"
        ), f"Expected 'hello', got {conv._content_txt!r}"
    ...
    def test_process_blocks_multiple_text_chunks_accumulate(self) -> None:
        ...
        assert (
            len(conv._content) == 2
        ), f"Expected 2 content items, got {len(conv._content)}"
        assert (
            conv._content_txt == "hello world"
        ), f"Expected 'hello world', got {conv._content_txt!r}"
    ...
    def test_process_blocks_tool_call_block(self) -> None:
        ...
        msg = Message(
            ...
            chunks=[
                ToolCallBlock(
                    tool_call_id="call_1", tool_name="fn", tool_kwargs={"a": 1}
                )
            ],
        )
        conv = ChatMessageConverter(msg)
        conv._process_blocks()
        assert (
            len(conv._tool_call_dicts) == 1
        ), f"Expected 1 tool call, got {len(conv._tool_call_dicts)}"
    ...
    def test_process_blocks_document_via_dispatch(self) -> None:
        ...
        assert (
            conv._content[0]["type"] == "file"
        ), f"Expected type 'file', got {conv._content[0]['type']}"
    ...
    def test_process_blocks_empty_chunks(self) -> None:
        ...
        assert (
            conv._content_txt == ""
        ), f"Expected empty text, got {conv._content_txt!r}"
        assert (
            conv._tool_call_dicts == []
        ), f"Expected empty tool calls, got {conv._tool_call_dicts}"
    ...
    def test_resolve_content_string_for_assistant(self) -> None:
        ...
        assert (
            conv._resolve_content() == "answer"
        ), "Assistant should get string content"
    ...
    def test_resolve_content_string_for_user_only_text(self) -> None:
        ...
        conv._content_txt = "hello"
        assert (
            conv._resolve_content() == "hello"
        ), "User with only text should get string"
    ...
    def test_resolve_content_none_for_assistant_with_tool_calls(self) -> None:
        ...
        assert (
            conv._resolve_content() is None
        ), "Empty assistant with tool calls should be None"
    ...
    def test_resolve_content_empty_string_without_tool_calls(self) -> None:
        ...
        conv._content_txt = ""
        assert (
            conv._resolve_content() == ""
        ), "Empty assistant without tools keeps empty string"
    ...
    def test_assemble_includes_tool_calls(self) -> None:
        ...
        conv._content_txt = ""
        conv._tool_call_dicts = [
            {"id": "c1", "type": "function", "function": {"name": "fn"}}
        ]
        result = conv._assemble()
        assert "tool_calls" in result, "Expected tool_calls key in result"
        assert (
    ...
    def test_merge_legacy_tool_calls_when_no_blocks(self) -> None:
        ...
        assert (
            result["tool_calls"] == legacy_calls
        ), f"Unexpected tool_calls: {result.get('tool_calls')}"
    ...
    def test_merge_legacy_skipped_when_tool_call_blocks_exist(self) -> None:
        ...
        conv._merge_legacy_kwargs(result)
        assert (
            "tool_calls" not in result
        ), "Legacy should not be merged when blocks exist"
    ...
    def test_merge_tool_call_id_passthrough(self) -> None:
        ...
        assert (
            result["tool_call_id"] == "call_42"
        ), f"Expected 'call_42', got {result.get('tool_call_id')}"
    ...
    def test_build_audio_path_skips_process_blocks(self) -> None:
        ...
        assert result["audio"] == {
            "id": "aud_1"
        }, f"Unexpected audio: {result.get('audio')}"
        assert (
            conv._content == []
        ), "Content should be empty — _process_blocks was skipped"
...
class TestResponsesMessageConverterMethods:
    ...
    def test_process_blocks_text_chunk_assistant_role(self) -> None:
        ...
        conv._process_blocks()
        assert (
            conv._content[0]["type"] == "output_text"
    ...
    def test_process_blocks_thinking_block_with_content(self) -> None:
        ...
        msg = Message(
            ...
            chunks=[
                ThinkingBlock(
                    content="thinking hard", additional_information={"id": "t1"}
                ),
                TextChunk(content="answer"),
            ],
        )
        ...
        conv._process_blocks()
        assert (
            len(conv._reasoning) == 1
        ), f"Expected 1 reasoning item, got {len(conv._reasoning)}"
        assert (
            conv._reasoning[0]["type"] == "reasoning"
        ), f"Expected reasoning type, got {conv._reasoning[0]['type']}"
        assert (
            conv._reasoning[0]["id"] == "t1"
        ), f"Expected id 't1', got {conv._reasoning[0]['id']}"
    ...
    def test_process_blocks_tool_call_block(self) -> None:
        ...
        assert (
            conv._tool_call_dicts[0]["type"] == "function_call"
    ...
    def test_process_blocks_document_via_dispatch(self) -> None:
        ...
        assert (
            conv._content[0]["type"] == "input_file"
    ...
    def test_assemble_tool_output_with_call_id_fallback(self) -> None:
        ...
        result = conv._assemble_tool_output()
        assert (
            result["call_id"] == "call_99"
        ), f"Expected call_id 'call_99', got {result['call_id']}"
    ...
    def test_assemble_message_dict_basic(self) -> None:
        ...
        assert (
            result["role"] == "assistant"
        ), f"Expected assistant role, got {result['role']}"
        assert (
            result["content"] == "answer"
        ), f"Expected 'answer', got {result['content']}"
    ...
    def test_assemble_message_dict_with_reasoning_returns_list(self) -> None:
        ...
        assert isinstance(
            result, list
        ), f"Expected list when reasoning present, got {type(result)}"
        assert len(result) == 2, f"Expected 2 items, got {len(result)}"
        assert (
            result[0]["type"] == "reasoning"
        ), f"Expected reasoning first, got {result[0].get('type')}"
        assert (
            result[1]["role"] == "assistant"
        ), f"Expected message dict second, got {result[1].get('role')}"
    ...
    def test_assemble_message_dict_rewrites_system_to_developer(self) -> None:
        ...
        assert (
            result["role"] == "developer"
        ), f"Expected developer role, got {result['role']}"
    ...
    def test_assemble_message_dict_strips_none_with_drop_none(self) -> None:
        ...
        assert (
            "content" not in result
        ), f"Expected 'content' stripped, got keys: {list(result.keys())}"
    ...
    def test_assemble_legacy_tool_calls_with_reasoning(self) -> None:
        ...
        assert (
            result[1]["id"] == "lg_1"
        ), f"Expected legacy call second, got {result[1]}"
    ...
    def test_assemble_tool_call_dicts_path(self) -> None:
        ...
        assert isinstance(result, list), f"Expected list, got {type(result)}"
        assert (
            result[0]["type"] == "function_call"
        ), f"Expected function_call, got {result[0].get('type')}"
    ...
    def test_assemble_plain_text_user_path(self) -> None:
        ...
        ), f"Expected 'hello', got {result.get('content')}"
    ...
    def test_assemble_default_message_dict_path(self) -> None:
        ...
        assert (
            result["role"] == "assistant"
        ), f"Expected assistant role, got {result.get('role')}"
    ...
    def test_assemble_user_mixed_blocks_not_plain_text(self) -> None:
        ...
        assert isinstance(
            result["content"], list
        ), f"Expected list content, got {type(result['content'])}"
    ...
    def test_build_populates_state_and_returns(self) -> None:
        ...
        assert (
            result["content"] == "test"
        ), f"Expected 'test', got {result.get('content')}"
        assert len(conv._content) == 1, "Expected _process_blocks to populate _content"
        assert (
            conv._content_txt == "test"
        ), f"Expected 'test' in _content_txt, got {conv._content_txt!r}"
    ...
    def test_build_assistant_with_tool_calls(self) -> None:
        ...
        msg = Message(
            ...
            chunks=[
                ToolCallBlock(
                    tool_call_id="c1", tool_name="search", tool_kwargs='{"q": "x"}'
                )
            ],
        )
        conv = ResponsesMessageConverter(msg)
        ...
        assert (
            result[0]["call_id"] == "c1"
        ), f"Expected call_id 'c1', got {result[0].get('call_id')}"
    ...
    def test_build_system_always_rewritten_to_developer(
